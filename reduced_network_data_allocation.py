# -*- coding: utf-8 -*-
"""
Revised v2: reorder loops for efficiency
- All outputs go to Data/data_allocation
- loop order and where computations occur were adjusted to avoid redundant work
"""
...
for NN in NODE_NUMBER:
    ...
    # Annual-average NG price per bus, then expand back to daily length
    NG_prices_bus_avg = NG_prices_all.mean(axis=0)
    NG_prices_daily_avg = pd.DataFrame([NG_prices_bus_avg] * len(NG_prices_all), columns=NG_prices_all.columns)
    ...
    # Annual-average Coal price per bus, then expand back to daily length
    Coal_prices_bus_avg = Coal_prices_all.mean(axis=0)
    Coal_prices_daily_avg = pd.DataFrame([Coal_prices_bus_avg] * len(Coal_prices_all), columns=Coal_prices_all.columns)
    ...
    # precompute which reduced buses actually have wind/solar capacity
    ...
    for Tp_pct in trans_p:
        ...
        cols_order = ['line'] + unique_nodes_lbl
        df_line_to_bus = df_line_to_bus[cols_order]
        fn_line_to_bus = f'line_to_bus_{NN}_tp_{Tp_pct}.csv'
        full_line_to_bus_path = _save_csv(df_line_to_bus, fn_line_to_bus)
        ...
        for UC in UC_TREATMENTS:
            for y in years:
                ...
                y_string = str(y)
                fn_load = 'Load/synthetic/BA_load_corrected_' + y_string + '.csv'
                df_load = pd.read_csv(os.path.join(base_dir, fn_load), header=0, index_col=0)
                fn_wind = 'Gen/synthetic/BA_wind_' + y_string + '.csv'
                df_wind = pd.read_csv(os.path.join(base_dir, fn_wind), header=0,index_col=0)
                fn_solar = 'Gen/synthetic/BA_solar_corrected_' + y_string + '.csv'
            ...
            # GENERATOR PARAMS (full + aggregated oil)
                ...
                for _, row in thermal_gens_info.iterrows():
                    if row['typ'] == 'ngcc':
                        gen_fuel_price = NG_prices_daily_avg.loc[:, row['node']].copy()
                    elif row['typ'] == 'coal':
                        gen_fuel_price = Coal_prices_daily_avg.loc[:, row['node']].copy()
                    elif row['typ'] == 'oil':
                ...
                copy('east_19_lostcap.csv', data_allocation_dir)
